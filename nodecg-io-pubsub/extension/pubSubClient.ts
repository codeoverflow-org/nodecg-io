...
export class PubSubServiceClient implements ServiceClient<PubSubClient> {
    ...
    async onRedemption(callback: (message: PubSubRedemptionMessage) => void): Promise<PubSubListener<PubSubRedemptionMessage>> {
        return await this.client.onRedemption(this.userid, callback);
    }

    async onSubscription(callback: (message: PubSubSubscriptionMessage) => void): Promise<PubSubListener<PubSubSubscriptionMessage>> {
    ...
    async onBitsBadgeUnlock(callback: (message: PubSubBitsBadgeUnlockMessage) => void): Promise<PubSubListener<PubSubBitsBadgeUnlockMessage>> {
        return await this.client.onBitsBadgeUnlock(this.userid, callback);
    }

    async onModAction(channelID: string, callback: (message: PubSubChatModActionMessage) => void): Promise<PubSubListener<PubSubChatModActionMessage>> {
